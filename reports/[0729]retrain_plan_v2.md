# [2026-07-29] 재학습 계획 v2 — LPIPS 실효 세기를 run2 수준으로 되돌려 머릿결 방향 노이지 원인 확정

## 0. 요약

| | 내용 |
|---|---|
| **관측** | 세 run의 설정 `w_lpips`는 모두 0.1인데 **실효 세기 `R`만 55배 차이**(run2 ≈0.018 → run3 1.016), `R`이 오른 run3에서 머릿결 방향이 어긋남 |
| **반증** | run3 ep10은 LPIPS가 켜지기 전인데 이미 어긋나 있음 → `R`만으로는 설명 안 됨(§2-3) |
| **이번 run** | **Run A** = unbraid phase1 + `R ≈ 0.02` + flow matte 가중 `m²→m` 복원 |
| **비용** | 15 epoch ≈ 2.1h / 20 epoch ≈ 3.0h / 40 epoch ≈ 6.5h + val 오버헤드 (§3) |
