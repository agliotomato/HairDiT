...
| Ours / Ours+Gate / Sketch-only / Sketch-only+Gate / Raw-only / Matte-CNN-only | 공식 명명 = 내부 mcs1 / mcs2 / mcs3 / mcs4 / mcs5 / mcs6 (논문 본문엔 mcs 코드 금지) |
...
- 🔴 **모든 시나리오 (T1·T2·T3·T4·T5·T6) 에서 mcs1 ~ mcs6 6구성 전체 진행** (앞서 한 실험·향후 실험 모두). 각 시나리오의 **핵심 비교 쌍**(예: T6 = Ours vs Ours+Gate)은 시나리오 설명에 명시된 그대로 유지하되, 나머지 mcs 구성도 같은 입력으로 함께 추론·측정해 보고서에 포함.
...
- **비교**: **mcs1 ~ mcs6 전체 6구성** (핵심 쌍: Sketch-only vs Ours).
...
- **비교**: **mcs1 ~ mcs6 전체 6구성** (핵심 쌍: Ours / Matte-CNN-only / Raw-only / **Sketch-only(floor)** 4구성).
- **측정**: **region IoU**.
...
- **비교**: **mcs1 ~ mcs6 전체 6구성** (핵심 쌍: Ours vs Ours+Gate).
...
### T4. Shading 보존 (T6 보조)
- **입력**: 타겟 **A** + recolor 스케치. **비교**: **mcs1 ~ mcs6 전체 6구성** (핵심 쌍: Ours vs Ours+Gate).
...
- **비교**: **mcs1 ~ mcs6 전체 6구성** (핵심 쌍: Ours vs Raw-only).
...
- **입력**: 타겟 A 또는 B, 뚜렷한 다색 sparse 스케치. **비교**: **mcs1 ~ mcs6 전체 6구성** (핵심 쌍: Ours vs Ours+Gate).
