## 논문과 코드의 Loss 수식 불일치 (L_flow · L_edge)
...
## 수정한 사항
### 스케치 컬러 (stroke recolor)

스케치의 각 stroke를 실제 헤어색으로 다시 칠하는(recolor) 과정. 색은 모두 `[0,1]` float.
**① 같은 stroke 묶기 → ② 그 위치의 헤어색으로 대표색 결정 → ③ 대표색이 너무 어두우면 floor로 상향**.

#### ① stroke grouping — 양자화 제거

* **기존** : float를 uint8로 바꾼 뒤 **5-bit 양자화**하여, 같은 양자화 색을 한 group으로 묶음
  * 양자화 탓에 살짝 다른 두 stroke가 한 group으로 합쳐지는 오류 가능
* **수정** : 원본 8-bit에서 **완전히 같은 RGB끼리** group (양자화 없음), `(0,0,0)` 배경은 제외
  * flat-color(안티에일리어싱 없는 순색) 스케치라 exact RGB로 stroke가 정확히 분리됨

#### ② 대표(머리)색 결정

각 stroke group 위치에서 그 group을 칠할 대표색을 정한다.

* **기존** : `target = img×matte`에서 3채널 밝기합 `> 0.05`인 픽셀을 대상. 유효 픽셀 부족 시 **원본 스케치색 유지**
* **수정** : **matte 내부(= 실제 헤어 영역)의 원본 이미지 픽셀**을 대상 (SHS 방식: 원본 헤어 RGB로 칠함)
  * 대표색 = train은 1/3 랜덤 픽셀·2/3 평균, infer는 평균 (이 선택 규칙 자체는 기존과 동일)
  * matte 내부 유효 픽셀 `< 10`개면 group 전체를 **검정 처리**
  * matte 바깥 stroke 픽셀은 원색 유지가 아니라 **검정으로 지움** (matte = 칠하는 canvas)

#### ③ 최소색 floor 

대표색이 너무 어두우면 배경 `(0,0,0)`과 구분되지 않으므로, 조건을 만족할 때만 최소색(floor)으로 상향.

* **기존** : 최소색 `28/255`, 조건 `max < 28/255`, 만족 시 색 전체를 `(28,28,28)`로 **통째 교체**
  `sampled_color = (28,28,28)/255   if  max(sampled_color) < 28/255`
* **수정** : 최소색 `[7,7,10]/255`, 조건 `max < 10/255` (floor의 max 채널), **채널별 상향**
...
* 예: `(9,9,9) → (9,9,10)` (기존은 `(7,7,10)`로 R,G가 깎임), `(0,0,0) → (7,7,10)`
...
1. **스케치 latent (16채널)** : 색이 칠해진 스케치를 (학습하지 않는) VAE로 인코딩 
2. **matte 보정 (16채널)** : matte를 작은 CNN(matte CNN)에 통과시켜 헤어 영역에만 작동하는 보정 신호를 만들고 스케치 latent에 더함. 이 보정은 처음엔 0에서 시작해 필요한 만큼만 학습됨
3. **위치 anchor (16채널)** : matte의 모양·위치를 담는 신호 (아래 pixel unshuffle 과정)
4. 최종 조건 = (스케치 latent + λ·matte 보정) ⊕ (위치 anchor) = **32채널**  [Eq. 6]


**matte를 latent 크기로 줄이는 과정 **

matte(512×512)를 단순 축소하지 않고, 8×8 국소 점유(occupancy)를 보존하도록 아래를 거친다.

* **PixelUnshuffle** : 8×8 픽셀 블록을 채널 방향으로 접음 → 위치 anchor는 이걸 16채널로 투영 
* **Channel averaging** : 접힌 블록을 평균 → 64×64 latent matte
* **AvgPool 2×2 → flatten** : 32×32로 줄인 뒤 펼쳐 1024개 token으로 
* 이 matte token으로 ControlNet 잔차를 헤어 영역에 게이팅 : `r̂ = r ⊙ [a·matte_tok + (1−a)]` (a=게이팅 강도)
