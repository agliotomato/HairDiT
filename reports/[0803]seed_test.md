...
* **GT 오차 평균**: 위 표(σ_i=3) 기준 두 이미지 모두 mcs2 < run2p1 < run4. run4 가 최고값인
  ...
  run2p1 순서는 `σ_i` ≤ 5(9/15 조합)에서 두 이미지 모두 유지되나, `σ_i` ≥ 6 에서는 CM_1067 만
...
* **GT 자체가 실사진이라 stroke 를 따라 일방적으로 흐르지 않음.** 웨이브·레이어 겹침 때문에
  ...
  CM_1082 9.91° 로, **측정 오차의 62~64% 가 GT 자체의 복잡도**(모든 run 에 동일하게 가산돼 §5.2 의 순위 비교엔 무영향). 절대값은 모델 정확도 지표가 아니며, 지침에는 없던 내용으로 이번 검증 과정에서 발견.
