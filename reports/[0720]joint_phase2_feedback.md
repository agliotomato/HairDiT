[reports/[0718]results.md](%5B0718%5Dresults.md) 결과에 대한 교수님 피드백을, 학습 로그·코드·정량 측정으로 검증한 리포트.

## 0. 요약 (TL;DR)

1. **phase2는 epoch가 갈수록 나빠짐** — 색 반영↓, 질감 푸석 (§1 그림).  
2. 그런데 **학습 loss는 계속 감소** → **loss ≠ 시각 품질** (§2).  
3. mcs 학습 이후 loss 코드를 2곳 바꿈: **edge threshold**·**flow 정규화** (§3).  
   - **edge loss 변경은 무해** (마스크 <0.2% 변화, §3-1).  
   - **flow 정규화 변경이 진짜 원인** — flow gradient가 ~60배 커지며 **LPIPS/edge가 사실상 무력화**(flow/lpips 0.9×→55×, §3-2).  
4. **재학습 제안: loss 밸런스 복원** (§4).  
...
→ **loss는 계속 내려가는데 육안·지표는 phase2 후반에 나빠짐. 즉 목적함수(flow+LPIPS+edge)가 지각적 색·질감을 제대로 측정하지 못하고 있음.** 왜 그런지는 §3.
