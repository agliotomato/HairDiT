...
(seed 완전 강건)까지는 못 미친다. 해석과 다음 단계(재학습 여부)는 §6·§7 참고 — **재학습
착수는 이 보고서에서 결정하지 않았다.**

---

## 1. 전처리 — §3 검증 중 발견한 이슈와 처리

가이드 §2 코드 그대로 `data/test/sketch_gt`(= `data/paper/sketch_gt`와 동일 파일, cmp로 확인)
CM_1067·CM_1082 에 K=25/15/11(L1/L2/L3) 적용.
...
### 1.2 §3 시각화 검증 — 판단이 갈린 지점

체크리스트 4개 중 2개가 애매해 사용자와 상의 후 처리:
...
- **matte 경계 유착(#2)**: 바깥쪽 실루엣을 따라 얇게 흐르는 stroke 발견. `matte_erode`를
  ...
  밀도만 감소해, **사용자 확인 하에 가이드 기본값(`boundary_exclude=0`) 유지**로 결정.
...
기존 seed 실험과 동일 조건(체크포인트 run4 phase1 epoch30, config `joint_phase1.yaml`,
20-step, BLD full/soft18, pixel-blend α0.75)에서 sketch 입력만 교체. baseline·mcs2 참조는
기존 렌더(`outputs/0803/seed_run4`, `seed_mcs2`) 재사용, L1/L2/L3 24장만 신규 추론.
...
### 3.3 방향 시각화 (seed42, 방향=색상/coherence=채도)

<img src="../outputs/0804/orientation_viz/viz_CM_1067_seed42.png" width="420"> <img src="../outputs/0804/orientation_viz/viz_CM_1082_seed42.png" width="420">
...
매끈해진다 — coherence 수치 상승(0.751→0.809)과 일치. CM_1082는 육안상 조건 간 차이가
뚜렷하지 않다 — 표의 수치 개선폭도 CM_1067보다 작다.
